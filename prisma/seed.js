...
const rolePermissionCodes = {
  ...
  trainer: ['course.create', 'course.read', 'course.update', 'course.publish', 'quiz.create', 'quiz.grade'],
...
async function seedUsers(departments, roles) {
  ...
  const usersData = [
    ...
    { fullName: 'Charlie Student', email: 'student3@example.com', positionTitle: 'Designer', departmentId: departments[2].id, roleId: employeeRole.id },
    { fullName: 'David Student', email: 'student4@example.com', positionTitle: 'Marketing Specialist', departmentId: departments[3].id, roleId: employeeRole.id },
    { fullName: 'Eve Student', email: 'student5@example.com', positionTitle: 'Sales Representative', departmentId: departments[4].id, roleId: employeeRole.id },
    { fullName: 'Frank Student', email: 'student6@example.com', positionTitle: 'HR Manager', departmentId: departments[5].id, roleId: employeeRole.id },
    { fullName: 'Grace Student', email: 'student7@example.com', positionTitle: 'Data Analyst', departmentId: departments[9].id, roleId: employeeRole.id },
  ];
...
async function seedCourses(users, departments, categories, tags) {
  ...
  const coursesData = [
    ...
    { title: 'React Complete Guide', slug: 'react-complete-guide', description: 'Master React.js for modern web apps', categoryId: categories[1].id, trainerId: trainer1.id, deptId: departments[0].id, duration: 720, tagIds: [tags[0].id, tags[3].id] },
    { title: 'TypeScript Mastery', slug: 'typescript-mastery', description: 'Advanced TypeScript patterns and practices', categoryId: categories[0].id, trainerId: trainer2.id, deptId: departments[0].id, duration: 480, tagIds: [tags[1].id, tags[2].id] },
    { title: 'Docker & Kubernetes', slug: 'docker-kubernetes', description: 'Container orchestration and deployment', categoryId: categories[2].id, trainerId: trainer2.id, deptId: departments[0].id, duration: 540, tagIds: [tags[5].id] },
    { title: 'AWS Cloud Practitioner', slug: 'aws-cloud-practitioner', description: 'AWS fundamentals and best practices', categoryId: categories[5].id, trainerId: trainer1.id, deptId: departments[0].id, duration: 600, tagIds: [tags[6].id] },
    { title: 'Python for Data Science', slug: 'python-data-science', description: 'Data analysis with Python', categoryId: categories[3].id, trainerId: trainer2.id, deptId: departments[9].id, duration: 660, tagIds: [tags[4].id] },
    { title: 'PostgreSQL Database Design', slug: 'postgresql-database-design', description: 'Database design and optimization', categoryId: categories[0].id, trainerId: trainer1.id, deptId: departments[0].id, duration: 420, tagIds: [tags[7].id] },
    { title: 'Git & GitHub Workflow', slug: 'git-github-workflow', description: 'Version control best practices', categoryId: categories[2].id, trainerId: trainer2.id, deptId: departments[0].id, duration: 300, tagIds: [tags[9].id] },
    { title: 'MongoDB Essentials', slug: 'mongodb-essentials', description: 'NoSQL database fundamentals', categoryId: categories[0].id, trainerId: trainer1.id, deptId: departments[0].id, duration: 360, tagIds: [tags[8].id] },
    { title: 'Leadership Skills', slug: 'leadership-skills', description: 'Develop leadership and management skills', categoryId: categories[8].id, trainerId: trainer2.id, deptId: departments[5].id, duration: 480, tagIds: [] },
  ];
      ...
      for (let l = 1; l <= lessonCount; l++) {
        ...
        await prisma.lesson.create({
          data: {
            ...
            videoUrl: lessonType === 'video' ? `https://example.com/videos/${course.slug}-m${m}-l${l}.mp4` : null,
...
async function seedRoadmaps(departments, categories, users, courses) {
  ...
  const roadmapsData = [
    { title: 'Backend Developer Path', description: 'Complete backend development roadmap', targetPosition: 'Backend Developer', deptId: departments[0].id, categoryId: categories[0].id, courseIds: [courses[0].id, courses[2].id, courses[6].id] },
    { title: 'Frontend Developer Path', description: 'Modern frontend development roadmap', targetPosition: 'Frontend Developer', deptId: departments[0].id, categoryId: categories[1].id, courseIds: [courses[1].id, courses[7].id] },
    { title: 'Full Stack Developer Path', description: 'Complete full stack roadmap', targetPosition: 'Full Stack Developer', deptId: departments[0].id, categoryId: categories[0].id, courseIds: [courses[0].id, courses[1].id, courses[2].id, courses[6].id] },
    { title: 'DevOps Engineer Path', description: 'DevOps and cloud infrastructure', targetPosition: 'DevOps Engineer', deptId: departments[0].id, categoryId: categories[2].id, courseIds: [courses[3].id, courses[4].id, courses[7].id] },
    { title: 'Data Scientist Path', description: 'Data science and analytics', targetPosition: 'Data Scientist', deptId: departments[9].id, categoryId: categories[3].id, courseIds: [courses[5].id, courses[8].id] },
    { title: 'Cloud Architect Path', description: 'Cloud architecture and design', targetPosition: 'Cloud Architect', deptId: departments[0].id, categoryId: categories[5].id, courseIds: [courses[3].id, courses[4].id] },
    { title: 'Database Administrator Path', description: 'Database management and optimization', targetPosition: 'DBA', deptId: departments[0].id, categoryId: categories[0].id, courseIds: [courses[6].id, courses[8].id] },
    { title: 'Team Lead Path', description: 'Leadership and management skills', targetPosition: 'Team Lead', deptId: departments[5].id, categoryId: categories[8].id, courseIds: [courses[9].id] },
    { title: 'Software Engineer Path', description: 'General software engineering', targetPosition: 'Software Engineer', deptId: departments[0].id, categoryId: categories[0].id, courseIds: [courses[0].id, courses[2].id, courses[7].id] },
    { title: 'Mobile Developer Path', description: 'Mobile app development', targetPosition: 'Mobile Developer', deptId: departments[0].id, categoryId: categories[4].id, courseIds: [courses[1].id, courses[7].id] },
  ];
...
async function seedEnrollmentsAndProgress(users, courses) {
          ...
          if (completedLessons < targetCompleted) {
            await prisma.lessonProgress.create({
              data: {
                ...
                lastAccessedAt: new Date(Date.now() - Math.floor(Math.random() * 48) * 60 * 60 * 1000),
              },
            });
            completedLessons++;
            ...
            await prisma.lessonProgress.create({
              data: {
                ...
                lastAccessedAt: new Date(Date.now() - Math.floor(Math.random() * 2) * 60 * 60 * 1000),
              },
            });
            completedLessons++;
            break;
  ...
  console.log(`✅ Created ${enrollmentCount} enrollments, ${certificateCount} certificates, with lesson progress`);
}
  ...
  for (let i = 0; i < Math.min(students.length, roadmaps.length); i++) {
    ...
    await prisma.roadmapAssignment.create({
      data: {
        ...
        startedAt: progress !== 'assigned' ? new Date(Date.now() - (daysAgo - 2) * 24 * 60 * 60 * 1000) : null,
        completedAt: progress === 'completed' ? new Date(Date.now() - 1 * 24 * 60 * 60 * 1000) : null,
      },
    });
...
async function seedQuestionBanksAndQuestions(users, categories) {
  ...
  console.log(`✅ Created ${questionBanks.length} question banks, ${questionCount} questions, ${optionCount} options`);
...
async function seedQuizzesAndAttempts(courses, questionBanks, users) {
      ...
      for (let aq = 0; aq < quizQuestions.length; aq++) {
        ...
        if (question.questionType === 'essay') {
          ...
          if (question.questionType === 'single_choice') {
            ...
            const optionsToSelect = isCorrectAnswer 
              ? correctOptions 
  ...
  console.log(`✅ Created ${quizCount} quizzes, ${attemptCount} attempts, ${attemptQuestionCount} attempt questions, ${responseCount} responses, ${responseOptionCount} response options`);
}
  ...
  for (const enrollment of enrollments) {
    ...
    await prisma.learnerRiskAssessment.create({
      data: {
        ...
        recommendations: riskLevel === 'high' ? 'Immediate intervention needed' : 'Monitor progress',
