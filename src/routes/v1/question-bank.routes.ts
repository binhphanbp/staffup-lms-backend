...
router.patch(
  '/:bankId/questions/:id/deactivate',
...
router
  ...
  .all(validate(questionParamsSchema, 'params'))
...
router
  ...
  .post(
    validate(questionOptionParamsSchema.omit({ optionId: true }), 'params'),
...
router
  ...
  .all(validate(questionOptionParamsSchema, 'params'))
...
router
  .route('/:id')
  .all(validate(questionBankIdParamSchema, 'params'))
