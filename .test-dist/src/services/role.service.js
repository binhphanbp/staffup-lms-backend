...
class RoleService {
    ...
    static async getRoleById(id) {
        ...
        if (!role) {
            throw new utils_1.AppError('Role not found', 404);
        }
    ...
    static async createRole(data) {
        ...
        if (existingRole) {
            throw new utils_1.AppError('Role code already exists', 409);
        }
    ...
    static async updateRole(id, data) {
        ...
        if (existingRole.isSystem && data.code && data.code !== existingRole.code) {
            throw new utils_1.AppError('Cannot change the code of a system role', 400);
        }
        if (data.code && data.code !== existingRole.code) {
            const duplicateRole = await database_1.prisma.role.findUnique({
                where: { code: data.code },
                select: { id: true },
            });
            if (duplicateRole) {
        ...
        const permissions = data.permissionCodes !== undefined
            ? await this.resolvePermissions(data.permissionCodes)
            : undefined;
        const updatedRole = await database_1.prisma.$transaction(async (tx) => {
    ...
    static async deleteRole(id) {
        ...
        if (role.isSystem) {
            throw new utils_1.AppError('Cannot delete a system role', 400);
        }
        if (role._count.userRoles > 0) {
            throw new utils_1.AppError('Cannot delete a role that is currently assigned to users', 400);
        }
        await database_1.prisma.$transaction([
            database_1.prisma.rolePermission.deleteMany({
                where: { roleId },
            }),
            database_1.prisma.role.delete({
                where: { id: roleId },
            }),
        ]);
    }
    static buildRoleWhere(query) {
        const where = {};
        if (query.search) {
            where.OR = [
    ...
    static async resolvePermissions(permissionCodes) {
        ...
        if (permissions.length !== uniquePermissionCodes.length) {
            ...
            throw new utils_1.AppError(`Invalid permission code(s): ${missingCodes.join(', ')}`, 400);
        }
        return permissions;
    }
